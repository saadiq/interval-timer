...
export function assignColorsToWorkout(workoutData: WorkoutData): SectionWithColor[] {
  ...
  if (workoutData.type === 'amrap') {
    ...
    mainWorkoutColors = Array(workoutData.workout.rounds)
      .fill(
    ...
    // EMOM has one section per minute, each minute is 60 seconds
  ...
  const coloredSections: SectionWithColor[] = [
    ...
    ...getMainWorkoutSections(workoutData).map((section, index) => ({
      ...section,
      color: mainWorkoutColors[index] || 'bg-primary',
    })),
...
function getMainWorkoutSections(workoutData: WorkoutData): WorkoutSection[] {
  switch (workoutData.type) {
      ...
      return [
        {
          ...
          description: workoutData.workout.exercises
    ...
    case 'tabata':
      return Array(workoutData.workout.rounds)
        .fill(
        ...
        .flat();
    case 'emom': {
      // Create one 60-second section per minute
      const exerciseList = workoutData.workout.exercises
        .map(
          (ex) => ex.name.replace(/^\d+\s+/, '') // Remove leading numbers from exercise names
        )
        .join(', ');
