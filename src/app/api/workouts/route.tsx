...
export async function GET() {
  ...
  const workoutDetails = dates.reduce((acc, date) => {
    ...
    if (workout.type === 'circuit' || workout.type === 'amrap' || workout.type === 'tabata' || workout.type === 'emom') {
      const workoutExercises = workout.type === 'circuit' ? (workout as CircuitWorkout).workout.exercises :
                               workout.type === 'amrap' ? (workout as AMRAPWorkout).workout.exercises :
                               workout.type === 'tabata' ? (workout as TabataWorkout).workout.exercises :
    ...
    acc[date] = {
      ...
      totalDuration,
      exerciseCount: workout.type === 'circuit' ? (workout as CircuitWorkout).workout.exercises.length :
                     workout.type === 'amrap' ? (workout as AMRAPWorkout).workout.exercises.length :
                     workout.type === 'tabata' ? (workout as TabataWorkout).workout.exercises.length :
