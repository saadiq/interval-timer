...
export async function GET(req: NextRequest) {
  ...
  const warmUpTime = workout.data.warmUp.reduce(
    (total, exercise) => total + exercise.duration,
    0
  );
  let mainWorkoutTime = 0;
  const coolDownTime = workout.data.coolDown.reduce(
    (total, exercise) => total + exercise.duration,
    0
  );
  ...
  switch (workout.data.type) {
    case "circuit":
      mainWorkoutTime =
        (workout.data as CircuitWorkout).workout.exercises.reduce(
          (total, exercise) => total + (exercise.duration || 0),
          0
        ) * (workout.data as CircuitWorkout).workout.rounds;
      break;
    case "amrap":
    ...
    case "tabata":
      mainWorkoutTime =
        ((workout.data as TabataWorkout).workout.workDuration +
          (workout.data as TabataWorkout).workout.restDuration) *
        (workout.data as TabataWorkout).workout.rounds *
        (workout.data as TabataWorkout).workout.exercises.length;
      break;
    case "emom":
      mainWorkoutTime =
        (workout.data as EMOMWorkout).workout.rounds *
        (workout.data as EMOMWorkout).workout.exercises.length *
        ...
        >
          {workout.data.type === "circuit" && (
            ...
            >
              ...
              >
                Circuit
                {(workout.data as CircuitWorkout).workout.rounds > 1
                  ? ` (${(workout.data as CircuitWorkout).workout.rounds}x)`
                  : ""}
                :
              </span>
              {renderExercises(
                (workout.data as CircuitWorkout).workout.exercises,
                (exercise) =>
          ...
          {workout.data.type === "amrap" && (
            ...
            >
              ...
              >
                AMRAP (
                {formatTime((workout.data as AMRAPWorkout).workout.duration)}):
              </span>
              {renderExercises(
                (workout.data as AMRAPWorkout).workout.exercises,
          ...
          {workout.data.type === "tabata" && (
            ...
            >
              ...
              >
                Tabata
                {(workout.data as TabataWorkout).workout.rounds > 1
                  ? ` (${
                      (workout.data as TabataWorkout).workout.rounds
                    } rounds)`
                  : ""}
              ...
              {renderExercises(
                (workout.data as TabataWorkout).workout.exercises,
          ...
          {workout.data.type === "emom" && (
            ...
            >
              ...
              >
                EMOM
                {(workout.data as EMOMWorkout).workout.rounds > 1
                  ? ` (${(workout.data as EMOMWorkout).workout.rounds} rounds)`
                  : ""}
                :
              </span>
              {renderExercises(
                (workout.data as EMOMWorkout).workout.exercises,
                (exercise) => exercise.name
              )}
            </div>
          )}
