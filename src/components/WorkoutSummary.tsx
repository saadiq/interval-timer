...
export const WorkoutSummary: React.FC = () => {
  ...
  const isSectionActive = (section: SectionWithColor, index: number) => {
    ...
    if (index < warmUpLength || index >= workout.sections.length - coolDownLength) {
      ...
      return (
        section.name === currentSection.name && workout.sections.indexOf(currentSection) === index
      );
    } else {
  ...
  const renderCircuitSummary = (circuitWorkout: CircuitWorkout) => {
    ...
    const totalCircuitTime =
      exercises.reduce((total, ex) => total + (ex.duration || 0), 0) * rounds;
          ...
          <ul className="space-y-1">
            {exercises.map((exercise, index) => {
              ...
              return (
                <li
                  key={index}
                  className={`flex items-center justify-between ${isSectionActive(sectionWithColor, sectionIndex) ? 'bg-warning/20' : ''}`}
                >
                  <div className="flex items-center">
                    <span
                      className={`section-color-indicator ${sectionWithColor.color} w-4 h-4 rounded-full inline-block mr-2`}
                    ></span>
          ...
          <ul className="space-y-1">
            {exercises.map((exercise, exerciseIndex) => (
              <React.Fragment key={exerciseIndex}>
                <li
                  className={`flex items-center justify-between ${tabataSections[exerciseIndex * 2] && isSectionActive(tabataSections[exerciseIndex * 2], workout.data.warmUp.length + exerciseIndex * 2) ? 'bg-warning/20' : ''}`}
                >
                  <div className="flex items-center">
                    <span
                      className={`section-color-indicator ${tabataSections[exerciseIndex * 2]?.color ?? ''} w-4 h-4 rounded-full inline-block mr-2`}
                    ></span>
                    <ClickableMovementName name={exercise.name} />
                  </div>
                ...
                <li
                  className={`flex items-center justify-between ${tabataSections[exerciseIndex * 2 + 1] && isSectionActive(tabataSections[exerciseIndex * 2 + 1], workout.data.warmUp.length + exerciseIndex * 2 + 1) ? 'bg-warning/20' : ''}`}
                >
                  <div className="flex items-center">
          ...
          <ul className="space-y-1">
            {amrapSection.exercises.map((exercise: BaseExercise, index: number) => (
              <li key={index} className="flex items-center justify-between">
                <div className="flex items-center">
                  <span
                    className={`section-color-indicator ${amrapColor} w-4 h-4 rounded-full inline-block mr-2`}
                  ></span>
                  <ClickableMovementName name={exercise.name} />
                </div>
                <div>{exercise.reps !== undefined && <span>{exercise.reps} reps</span>}</div>
              </li>
            ))}
          </ul>
        </div>
          ...
          <ul className="space-y-1">
            {exercises.map((exercise, index) => (
              <li key={index} className="flex items-center justify-between">
                <div className="flex items-center">
                  <span
                    className={`section-color-indicator ${emomColor} w-4 h-4 rounded-full inline-block mr-2`}
                  ></span>
                  <ClickableMovementName name={exercise.name} />
                </div>
              </li>
  ...
  const renderSectionGroup = (
    sections: ReadonlyArray<SectionWithColor>,
    title: string,
    startIndex: number
  ) => {
          ...
          <ul className="space-y-1">
            {sections.map((section, index) =>
              renderSection(
                section,
                startIndex + index,
                isSectionActive(section, startIndex + index)
              )
            )}
          </ul>
        </div>
  ...
  const renderSection = (section: SectionWithColor, index: number, isActive: boolean) => {
    return (
      <li
        key={index}
        className={`flex items-center justify-between ${isActive ? 'bg-warning/20' : ''}`}
      >
        <div className="flex items-center">
          <span
            className={`section-color-indicator ${section.color} w-4 h-4 rounded-full inline-block mr-2`}
          ></span>
  ...
  return (
    <div
      className="bg-card border border-border rounded-lg shadow-xl p-6"
      role="region"
